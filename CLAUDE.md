...
**当前状态（09-02）：「分页」串讲完成（数据流 + 两条SQL + Lambda组装vs占位符绑定 + 拦截机制，动手加 size 上限防护并抽 limitPageSize 消除重复）✅，待做：串讲「图片上传」→ 测开模拟面试**
...
| 3 | 图片上传"为什么"：UUID、防路径穿越（08-10 已学）| ⏳ |

### 后续建议
- 上午：补复习剩余 2 个点（分页、图片上传）+ 投简历
- 下午：投简历 + 面试题（1-2h）
- 晚上：八股 + 力扣（2h）

### 模拟面试（用户要求：留到最后，现在先不做）
- ⏰ 时机：等全部剩余亮点（分页、图片上传）复习完再做
- 🎭 形式：Claude 扮演「中厂面试官」，不仅问项目，还要查八股文
- 🎯 目标：检验复习效果 + 暴露知识盲区

### 下次启动步骤（09-02 更新）
1. 读 CLAUDE.md 了解进度
2. 「分页」已串讲完成（09-02）：数据流 / 两条SQL / Lambda组装vs占位符绑定 / 拦截机制 + size 上限防护动手实践 ✅
3. ⚠️ 遗留：ItemServiceImpl 里 `prilimitPageSize` → `limitPageSize` 改名未做（三处：定义 + page + pageByUserId），明天先让用户自己改
4. 下一步：串讲「图片上传」（UUID 重命名 / 防路径穿越 / multipart）
5. 之后：测开模拟面试压轴（若暴露深度不够，再补 JMeter 压测报告加分项）
