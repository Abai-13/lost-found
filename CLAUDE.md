...
| 08-15 | 🔄 全流程复习：注册登录→发布→查询→状态修改→AI 整条链路串通；乐观锁从"漏讲"到彻底过关；客户端/服务端、限流vs降级、chat vs query 分清楚 | 🟢 掌握 | ✅ 能讲 |
| 08-18 | 🔄 复习：Docker 部署全套（多阶段构建/构建缓存/健康检查+启动顺序/数据卷持久化/服务名通信）+ 补地基（Docker 是什么、镜像/容器/Dockerfile/compose 四件套） | 🟢 掌握 | ✅ 能讲 |
| 08-23 | 🔄 复习：Redis 缓存三兄弟（穿透/击穿/雪崩：概念+防御+落地）+ 🔧 穿透代码修复（删 disableCachingNullValues + getById 返回 null + controller 判空） | 🟢 掌握 | ✅ 能讲 |
| 08-31 | 🆕 测开方向启动：测试简历 PDF + 测试八股（13-测试开发.md）+ 制定测开学习计划 | 🟢 完成 | — |
| 08-31 | 🔧 测开 Step1：JUnit 单元测试 11 个通过（BCrypt 加密、越权防护、乐观锁重试）+ 后端薪资改 12-16K | 🟢 完成 | ✅ 能讲 |
| 09-01 | 🔧 测开 Step1 完成：MockMvc 接口测试 10 个通过（UserController 4 + ItemController 6，覆盖 JWT 拦截器 401/200、multipart 发布）；并揪出修复 2 个真实 bug（application.yml 重复 spring key、Result.ok 重载坑） | 🟢 完成 | ✅ 能讲 |
| 09-02 | 🔄 串讲分页：数据流 / 两条SQL（COUNT+LIMIT）/ Lambda组装 vs 占位符绑定 / 拦截机制 | 🟢 掌握 | ✅ 能讲 |
| 09-02 | 🔧 动手实践：size 上限防护（page + pageByUserId + 抽 limitPageSize 消除重复） | 🟢 完成 | ⚠️ 需练习 |
| 09-02 | 📝 面试点：private 封装 / void 引用传递（改同一对象） | 🟡 了解 | ⚠️ 需练习 |
| 09-05 | 🔧 前端从 0 到 1：Vue3 + Element Plus 5 页面（登录注册/列表/详情/发布/AI），前后端分离 + axios 拦截器带 token + Vite 代理跨域 + 路由守卫 | 🟢 完成 | ⚠️ 需复习 |
| 09-05 | 🎓 前端 6 课教学：项目结构 → .vue 三段式 → ref/v-model 响应式 → 数据流(拦截器/token/跨域) → 5 页面接口对照 → 动手改(列表页加状态筛选) | 🟢 完成 | ⚠️ 需复习 |
| 09-05 | 🔧 后端缓存降级：GracefulCacheErrorHandler（Redis 挂了缓存静默降级直查 DB，服务不崩，面试加分点） | 🟢 完成 | ✅ 能讲 |
| 09-05 | 📝 简历整合：5 版本合并成 1 份通用版（意向改软件开发/全栈、薪资 9-12K、加前端、去 AI 味），导出 PDF/HTML | 🟢 完成 | — |
| 09-05 | 📦 Git push：前端 + 缓存降级 + 接口测试推到 GitHub（含之前漏推的 2 个提交） | 🟢 完成 | — |

---

## ⚠️ 下次新对话必须从这里继续（重要）

**当前状态（09-05）：前端已从 0 到 1 完成（Vue3 + Element Plus 5 页面，6 课教学），后端加了缓存降级，简历整合成通用版并导出 PDF，全部推上 GitHub。待做：明天复习前端 6 课 + 全栈模拟面试（后端 + 前端 + 八股）**

### 08-23 复习战果（Redis 缓存三兄弟）
**穿透**
- ✅ 概念：查缓存和 DB 都不存在的数据；攻击者用海量不存在 id 打穿缓存直击 DB
- ✅ 两种防御：空值缓存（null + 短 TTL）/ 布隆过滤器（查询前拦截）
- ✅ 取舍理由：自增主键 + 校园场景 → 空值缓存够用，布隆 ROI 不划算
- 🔧 代码修复 3 处：RedisConfig 删 disableCachingNullValues；ItemServiceImpl.getById 返回 null；ItemController.detail 判空抛异常
- ⚠️ 纠正认知：原注释「禁用 null 值（防穿透）」写反了——禁用 null 缓存 = 关掉防穿透
- ⏳ 待实测：Redis 未运行，null 缓存运行时行为未验证

**击穿**
- ✅ 概念：热点 key 刚好过期，大量并发请求同时打 DB（区别穿透：数据存在）
- ✅ 防御：互斥锁（setnx + 过期 + finally 释放）/ 逻辑过期（先返回旧值异步刷新）
- ✅ 手动版 getById 伪代码跑通（查缓存 → 抢锁 → 查库 → 回填 → 释放 → sleep 重试）
- ⚠️ 认知：@Cacheable 裸用不防击穿；手动版别轻易替换注解（生产用 Redisson RLock）

**雪崩**
- ✅ 概念：大量 key 同时过期 / Redis 宕机，请求雪崩式打 DB（区别击穿：一群 key）
- ✅ 防御：随机 TTL（jitter）/ Redis 高可用（主从/哨兵/集群）/ 多级缓存降级
- ⚠️ 认知：项目 entryTtl 是固定 5 分钟非随机，理论上存在雪崩风险；量小可接受

### 08-15 复习战果
- ✅ 乐观锁彻底焊死（version 匹配不上 → rows=0 → 重试3次）
- ✅ 客户端/服务端 角色分清（后端位置不变、角色可变）
- ✅ 限流（拦截器）vs 降级（service try-catch）分清
- ✅ chat vs query 两个接口分清
- ✅ 纠正"前端异常处理器" → 后端全局异常处理器

### 剩余待复习（之前学过，还没串到）
| 优先级 | 内容 | 状态 |
|:---:|------|:---:|
| 1 | Redis 缓存三兄弟：穿透/击穿/雪崩 ✅（08-23 复习完成 + 穿透代码修复）| ✅ 完成 |
| 2 | 分页"为什么"：防注入、编译期检查 ✅（09-02 串讲完成）| ✅ 完成 |
| 3 | 图片上传"为什么"：UUID、防路径穿越（前端发布页已演示，面试能讲即可，不单独串讲）| ✅ 并入面试 |

### 后续建议（09-05）
- 明天：复习前端 6 课 + 「演示动作↔技术点」表 → 模拟面试
- 持续：投简历（通用版 PDF）+ 八股 + 力扣

### 模拟面试（明天做，压轴）
- ⏰ 时机：09-06，用户先一口气复习完前端 6 课 + 「演示动作↔技术点」表，再开始
- 🎭 形式：Claude 扮演「中厂面试官」，边问项目（后端+前端）边查八股文
- 🎯 目标：检验复习效果 + 暴露知识盲区
- 📋 覆盖：后端（分层/JWT/分页/权限/AI/Redis三兄弟/乐观锁）+ 前端（结构/三段式/响应式/token管理/跨域）+ 八股

### 下次启动步骤（09-05 更新）
1. 读 CLAUDE.md 了解进度
2. 前端已完成（09-05）：Vue3 + Element Plus 5 页面，6 课已教完；用户说「明天一口气复习完再模拟面试」
3. 明天的核心：全栈模拟面试（见上「模拟面试」节）
4. 复习重点：前端 6 课 + 「演示动作↔技术点」表（注册登录→token 拦截器 / 发布→multipart+白名单 / 列表→LambdaQueryWrapper / 详情→两层防护 / AI→超时降级）
5. 遗留（已并入面试，不单独串讲）：图片上传"为什么"（UUID/防路径穿越）——前端发布页已演示，面试能讲即可
6. 可选加分项：装 Redis 后做「缓存命中/QPS 对比」压测报告

---

## 🧪 测开学习计划（08-31 新增）

> 目标：**后端为主 + 测开平行/保底**。测开已启动（08-31），剩余后端复习点（分页、图片上传）合并到测开末尾一起串。

### 为什么加测开
- 后端 15-25K 竞争激烈（八股深度 + 算法）；测开"会写代码的人少、算法 bar 略低"，更适合平行/保底投递
- 你的最大优势：会开发、能看懂代码、从根因定位 bug，这是纯黑盒测试候选人没有的差异化能力
- 已产出：测试简历 PDF（`resume/白晨鑫_测试开发_兰州理工大学.pdf`）+ 测试八股（`D:/八股资料/面试八股/13-测试开发.md`）

### 测开学习三步走（08-31 已启动）

> 剩余后端复习点「分页」「图片上传」为什么合并到测开？因为它们天然是测试素材——分页 = 接口测试的边界值用例设计，图片上传 = 安全测试的白名单/路径穿越。在测开阶段串讲，一举两得。

**第一步：补测试开发内容（代码落地）**
- JUnit 5 单元测试：测 Service 层逻辑（非发布者不能改状态、BCrypt 密码校验、缓存）
- 接口自动化测试：MockMvc 测 Controller 接口，把「注册→登录→发布→查询→改状态」主链路写成自动回归脚本
- （可选）JMeter 压测：物品列表接口加缓存前后 QPS 对比，产出性能报告

**第二步：测开八股（`13-测试开发.md`）**
- P0：测试基础理论 + 用例设计 + 经典 case 题（21 题中的前 11 题，必考命根子）
- P1：接口测试 + 自动化 + 性能 + 数据库/Linux
- P2：安全测试 + 综合题（「为什么做测试不做开发」是陷阱题，用"你会开发"这个优势答）

**第三步：测开模拟面试 + 投递**
- Claude 扮演测开面试官，围绕「项目测试视角 + 测开八股」提问
- 用「简历-测试开发版.md」投测开岗，期望薪资 10K-15K

### 测开完成标志
- ✅ 项目有 JUnit 单测 + 接口自动化测试（简历里「JUnit/Selenium/JMeter 学习中」改成「掌握」）
- ✅ 测开八股 P0 能对答如流
- ✅ 测开模拟面试过关
